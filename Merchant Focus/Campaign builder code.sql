...
--pai_mkt_history
--distinct date_time, campaign_type, campaign_category, account_id, bcookie
--json_text history event

2021-09-01 9AM  google SEO X Y 
2021-09-01 10AM  google SEO X Y 
2021-09-01 11AM  facebook SEO X Y 

select * from user_groupondw.dim_month;

select * from sandbox.rev_mgmt_deal_attributes;
select * from sandbox.rev_mgmt_deal_funnel rmdf sample 5;
...
drop table if exists np_w2l_mktg_acct_attrib_tmp purge;
create table np_w2l_mktg_acct_attrib_tmp as
  select
  ...
  when campaign_name__c like 'TXN1%' and campaign_name__c  like '%candaceholyfield_Gen__INS%' then 'Influencer_insta'
        -- when campaign_type = 'No Campaign - Referred' then 'Referral'
        when campaign_type in ('groupon-header-app1', 
                               'groupon-header1', 
                               'groupon-header-mweb1', 
                               'groupon-footer') then 'Groupon Header & Footer'
        when campaign_type in ('Google - Brand','Google - Remarketing',
                               'Google Brand - Advertising',
                               'Google Brand - Business',
                               'Google Brand - Contact Merchant',
                               'Google Brand - How To Business',
                               'Google Brand - Join',
                               'Google Brand - Merchant Misc',
                               'Google Brand - Number') then 'SEM-Brand'
        -- when campaign_type = 'Direct' then 'Direct'
        -- when campaign_type in ('Facebook F&D','Facebook HBW','Social Retargeting', 'Facebook Silver Lookalike') then 'Display'
        when campaign_type in ('Bing - Competitor','Google - Competitor','Google - Non-Brand','Google NB - Advertise','Google NB - Free Advertising','Google NB - Promote','Google NB - SB-Adv') then 'SEM-NB'
        -- when campaign_type = 'Phone Bank - No Campaign' then 'Phone Bank'
  ...
  when campaign_name__c like 'TXN1%' and campaign_name__c  like '%candaceholyfield_Gen__INS%' then 'Influencer_insta'
        ...
        when campaign_type in 
            ('Bing - Competitor',
             ...
             'Google - Non-Brand',
             'Google NB - Advertise',
             'Google NB - Free Advertising',
             'Google NB - Promote',
             'Google NB - SB-Adv') then 'SEM-NB'
        -- when campaign_type = 'Phone Bank - No Campaign' then 'Phone Bank'
        -- when campaign_type = 'Referral' then 'Referral'
        when campaign_type in ('Living Social','Merchant Blog/SBRC','Merchant Newsletter','Organic','Print') then 'SEO'
        else 'Direct / Referral / Other'
...
/*
select convertedaccountid, count(distinct campaign_paid_category) xyz, count(1) a 
from sandbox.pai_leads 
where max_value = 0
group by 1 having xyz = 1 and a > 1;

select 
   convertedaccountid, 
   sum(paid_first_last_touch) xyz
from sandbox.pai_leads
group by 1 
having xyz = 0;
...
select * 
from sandbox.pai_leads 
where convertedaccountid = '0013c00001tdYT1AAM'; */
   
   
   
--------------------PAI LEADS

--drop table sandbox.pai_leads;
--create multiset table sandbox.pai_leads as (
...
from 
sandbox.pai_leads
where campaign_group_lvl1_2 in ('SEO', 'Direct / Referral / Other', 'Groupon Header & Footer', 'Square Marketplace') and campaign_paid_category = 1;
--1253
select 
   distinct campaign_name 
from 
sandbox.pai_leads
where campaign_group_lvl1_2 in ('SEO', 'Direct / Referral / Other', 'Groupon Header & Footer', 'Square Marketplace') and campaign_paid_category = 0
--540
select 
   distinct campaign_name 
from 
sandbox.pai_leads
where campaign_group_lvl1_2 not in ('SEO', 'Direct / Referral / Other', 'Groupon Header & Footer', 'Square Marketplace') and campaign_paid_category = 1
--46
select 
   distinct campaign_name 
from 
sandbox.pai_leads
where campaign_group_lvl1_2 not in ('SEO', 'Direct / Referral / Other', 'Groupon Header & Footer', 'Square Marketplace') and campaign_paid_category = 0
------------------

select * from sandbox.pai_leads where lower(campaign_group_lvl1_2) like '%amz%';
...
SELECT 
case when lower(leadsource) like '%mia%' then 'mia' 
     when lower(leadsource) like '%metro%' then 'metro' end ld_source, 
count(1) xyz
FROM sandbox.pai_leads 
