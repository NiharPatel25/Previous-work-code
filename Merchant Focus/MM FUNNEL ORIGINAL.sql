...
set hive.groupby.orderby.position.alias=true;set hive.exec.parallel=true;use grp_gdoop_sup_analytics_db;drop table if exists sm_w2l_mktg_acct_attrib purge;


use grp_gdoop_bizops_db

select campaign_name__c, campaign_group2 , campaign_group1 
from grp_gdoop_bizops_db.np_w2l_mktg_acct_attrib_tmp 
group by campaign_name__c, campaign_group2 , campaign_group1
order by campaign_name__c, campaign_group2 , campaign_group1;


select campaign_name__c, campaign_group_lvl1_2, campaign_group_lvl1_1  
from sandbox.pai_leads
group by 1,2,3
order by 1,2,3;

select 
distinct campaign_name__c, campaign_group2 , campaign_group1 , campaign_type from 
grp_gdoop_bizops_db.np_w2l_mktg_acct_attrib_tmp 
where campaign_name__c = 'merchant_blog_gogbanner_perc_escape-room-marketing-advertising-ideas'
;
...
SET hive.exec.max.dynamic.partitions.pernode=3000;SET hive.tez.java.opts=-Xmx6000M;SET hive.tez.container.size=8192;
SET hive.exec.parallel=true;SET hive.vectorized.execution.enabled=true;SET hive.vectorized.execution.enabled=false;
SET hive.input.format=org.apache.hadoop.hive.ql.io.orc.OrcInputFormat;SET hive.enforce.sorting=true;SET hive.enforce.bucketing=true;
SET hive.enforce.bucketmapjoin=true;SET hive.enforce.sortmergebucketmapjoin=true;SET hive.optimize.bucketmapjoin=true;
...
SET hive.auto.convert.join.noconditionaltask.size=1000000000;SET hive.auto.convert.sortmerge.join=true;
SET hive.auto.convert.join.noconditionaltask = true;SET hive.optimize.skewjoin=true;SET hive.optimize.skewjoin.compiletime=true;
SET hive.skewjoin.mapjoin.map.tasks=10000;SET hive.skewjoin.key=10000;SET hive.exec.compress.output=true;SET mapred.input.compress=true;
SET hive.exec.compress.intermediate=true;SET mapred.output.compress=true;SET mapred.map.output.compression.codec=org.apache.hadoop.io.compress.SnappyCodec;
...
SET mapreduce.map.output.compress=true;SET mapreduce.map.output.compress.codec=org.apache.hadoop.io.compress.SnappyCodec;SET mapred.compress.map.output=true;
SET mapred.map.output.compression.codec=org.apache.hadoop.io.compress.SnappyCodec;SET mapred.output.compression.codec=org.apache.hadoop.io.compress.SnappyCodec;
SET mapred.input.compression.codec=org.apache.hadoop.io.compress.SnappyCodec;SET tez.am.container.reuse.enabled=true;
SET tez.session.am.dag.submit.timeout.secs=900;SET tez.am.container.session.delay-allocation-millis=900000;
SET hive.mapjoin.smalltable.filesize=1000000000;SET tez.runtime.shuffle.failed.check.since-last.completion=false;
SET hive.resultset.use.unique.column.names=false;set hive.exec.dynamic.partition.mode=nonstrict;
set hive.auto.convert.join=false;set hive.exec.parallel=true;SET mapred.reduce.tasks=253;SET mapreduce.job.reduces=253;
SET hive.mapjoin.smalltable.filesize=400000000;SET hive.optimize.sort.dynamic.partition=true;

select 
   campaign_name__c,
   campaign_group, 
   campaign_type,
   case when campaign_name__c like '%SEMN%' and campaign_name__c not like '%SEMNB%' then 1 else 0 end  new_log
from grp_gdoop_bizops_db.sm_w2l_mktg_acct_attrib
where campaign_name__c like '%SEMN%' and campaign_name__c not like '%SEMNB%' and campaign_group <> 'SEM-Brand'
limit 5
;
...
select 
   ...
   campaign_group, 
   campaign_type,
   case when campaign_name__c like '%SEMB%' then 1 else 0 end  new_log
from grp_gdoop_bizops_db.sm_w2l_mktg_acct_attrib
where campaign_name__c ='TXN1_UK__SEMB_GEN_EX_GOOG_TXT2'
limit 5
;
...
distinct
   ...
   campaign_group, 
   campaign_type,
   case when campaign_name__c like '%SEMN%' and campaign_name__c like '%VID%' then 1 else 0 end  new_log
from grp_gdoop_bizops_db.sm_w2l_mktg_acct_attrib
where split(campaign_name__c, '_')[3] = 'SEMNB'
limit 5
;
...
select
    ...
    from  user_groupondw.bld_events b
...
select * from grp_gdoop_bizops_db.sm_w2l_mktg_acct_attrib where highest_touch is not null;

------------------------------------------------STEP 2
sm_w2l_mktg_acct_attrib
    
    
    
    
    
drop table if exists grp_gdoop_sup_analytics_db.sm_w2l_mia_funnel;
create table sm_w2l_mia_funnel stored as orc as
  ...
  from (
    select
      ...
      , referrer_domain
      , case
          when utm_campaign like 'TXN1%' and split(utm_campaign, '_')[3] = 'SEMB' then 'SEM-Brand'-- made a change should be 4
          when utm_campaign like 'TXN1%' and split(utm_campaign, '_')[3] = 'SEMN' then 'SEM-NB'
          when utm_campaign like 'TXN1%' and split(utm_campaign, '_')[3] = 'DIS' then 'Display'
          when utm_campaign like 'TXN1%' and split(utm_campaign, '_')[3] = 'GPMC' then 'SEM-Brand'
          when utm_campaign like 'TXN1%' and split(utm_campaign, '_')[3] = 'SEMNB' then 'SEM-NB'
          when utm_campaign like 'TXN1%' and split(utm_campaign, '_')[3] = 'VID' then 'SEM-Brand'
          when utm_campaign like '%always_on%' then 'SEM-Brand'
          when utm_campaign  like '%SEMC %' then 'SEM-Brand'
          when utm_campaign like 'TXN1%' and utm_campaign  like '%candaceholyfield_Gen__FB%' then 'Influencer_fb'
          when utm_campaign like 'TXN1%' and utm_campaign  like '%candaceholyfield_Gen__INS%' then 'Influencer_insta'
 ...
 drop table if exists grp_gdoop_sup_analytics_db.sm_w2l_mia_traffic;
...
drop table if exists grp_gdoop_sup_analytics_db.sm_w2l_leads;
create table sm_w2l_leads stored as orc as
 ...
 drop table if exists grp_gdoop_sup_analytics_db.sm_w2l_launches;
...
(select 
    accountid,
    deal_uuid, 
    count(distinct campaign_group) xyz 
from grp_gdoop_bizops_db.np_temp_launch
group by accountid, deal_uuid) as fin
where xyz > 1;
 ...
 drop table if exists grp_gdoop_sup_analytics_db.smm_w2l_kpis;
...
-------------------exporting to raw sheet (WITHOUT THE BREAKDOWN)
  
drop table grp_gdoop_sup_analytics_db.np_temp_sheet_import;
use grp_gdoop_sup_analytics_db;
select
wk.week_end, 
...
, substr(dm.month_start, 1,10) report_mth
from smm_w2l_kpis c
     join user_dw.v_dim_day dd on date_format(c.week,'yyyy-MM-dd') = dd.day_rw
...
, case when country_code = 'US' then 'NAM' else 'INTL' end region
    ...
    , cast(report_date as date) - EXTRACT(DAY FROM cast(report_date as date)) + 1 month_start
...
select distinct wk_end, report_month 
from user_edwprod.agg_gbl_traffic 
where wk_end = '2022-09-04';

select * from user_groupondw.dim_month;
select * from user_dw.v_dim_day;
select * from grp_gdoop_sup_analytics_db.smm_w2l_kpis;
...
select 
   ...
   substr(dm.month_start, 1,10) report_mth2
from 
grp_gdoop_sup_analytics_db.smm_w2l_kpis c
     join user_dw.v_dim_day dd on date_format(c.week,'yyyy-MM-dd') = dd.day_rw
     join user_groupondw.dim_month dm on dd.month_key = dm.month_key
     where c.week > '2020-01-01' and dd.day_rw < current_date;
